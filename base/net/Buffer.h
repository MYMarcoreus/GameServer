...
public:
    ...
    /*! Buffer不实现来自套接字Socket的recv任务，因为对于recv任务，存在ET和LT的区别，这应该让其所有者TcpConnection实现（而send任务可以实现） !*/
    ...
    bool RetrieveDataIntoCBuffer(void* dest_buf, size_t data_len);
    ...
    bool RetrieveDataIntoPODStruct(T& dest)
    {
        return RetrieveDataIntoCBuffer(&dest, sizeof(dest));
    }
    ...
    std::string RetrieveAllDataAsString();
...
private:
    ...
    bool TestAndSetFreeSpace(int needLen);
