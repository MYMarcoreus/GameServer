...
void UdpSession::SendUDP(const Buffer &buf) {
...
void UdpSession::SendUDP(const std::shared_ptr<google::protobuf::Message> &message) {
    m_udpTran.SendUDP(message, m_peerAddr);

}

void UdpSession::SendUDP(const google::protobuf::Message &message) {
    m_udpTran.SendUDP(message, m_peerAddr);
