...
void UdpTransport::SendUDP(const std::string_view & message, IPAddressPtr peerAddr) {
    ...
    m_sendWorkThreadPool->PushTask([this, msg = std::string(message), peerAddr]() {
        this->SendUDPWorker(std::string_view{msg}, peerAddr);
    });
}

void UdpTransport::SendUDPWorker(const std::string_view & buf, IPAddressPtr peerAddr) { //! const &延长临时对象生命周期
