...
* a multiple of 4.
*/
static void __md5_Encode(unsigned char *output, unsigned int *input, unsigned int len)
{
    unsigned int i, j;
...
* a multiple of 4.
*/
static void __md5_Decode(unsigned int *output, unsigned char *input, unsigned int len)
{
    unsigned int i, j;
    for (i = 0, j = 0; j < len; i++, j += 4)
...
/*
* MD5 padding. Adds padding followed by original m_FullLength.
*/
static void __md5_Pad(MD5_CTX *context)
{
    ...
    /* Append m_FullLength (before padding) */
...
static void __md5_Transform(unsigned int state[4], unsigned char block[64])
{
...
void EncryptMD5str(char *output, unsigned char *input, int len)
{
    ...
    //sprintf_s(&output[i * 2], "%02X", strcrypt[i]);
}
