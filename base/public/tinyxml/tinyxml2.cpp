	...
	while( p && *p ) {
       ...
       const int initialLineNum = node->_parseLineNum;
        ...
        if ( ele ) {
            ...
            if ( mismatch ) {
                _document->SetError( XML_ERROR_MISMATCHED_ELEMENT, initialLineNum, "XMLElement m_TypeName=%s", ele->Name());
                _document->DeleteNode( node );
                break;
            }
        }
...
{
    // Parse using the m_TypeName rules: bug fix, was using ParseText before
    p = _name.ParseName( p );
    if ( !p || !*p ) {
        return 0;
    }
...
char* XMLElement::ParseAttributes( char* p, int* curLineNumPtr )
{
    ...
    while( p ) {
        p = XMLUtil::SkipWhiteSpace( p, curLineNumPtr );
        if ( !(*p) ) {
            _document->SetError( XML_ERROR_PARSING_ELEMENT, _parseLineNum, "XMLElement m_TypeName=%s", Name() );
            return 0;
        }
        ...
        if (XMLUtil::IsNameStartChar( (unsigned char) *p ) ) {
            ...
            if ( !p || Attribute( attrib->Name() ) ) {
                ...
                _document->SetError( XML_ERROR_PARSING_ATTRIBUTE, attrLineNum, "XMLElement m_TypeName=%s", Name() );
                return 0;
            }
