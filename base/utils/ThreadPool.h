...
class ThreadPool {
public:
    using Task = ::std::function<void ()>; //! 必须是`std::function<void ()>`，否则lambda函数/std::bind函数无法传入带捕获列表/多参数函数

public:
    ///@param thread_num 线程池中的线程数量，默认为CPU的核心数
    ///@param queueSize 任务队列的长度，默认为无限队列（其实为int的最大值）
    explicit ThreadPool(int queueSize = BoundedQueue<Task>::NO_BOUND_SIZE);

    ~ThreadPool();

    ///@brief 启动所有线程，但是并没有任务，所以一开始会wait任务
    void Start(int threadNum = (int) std::thread::hardware_concurrency() / 2);
...
private:
    BoundedQueue<Task>          m_Queue;     // 任务队列
