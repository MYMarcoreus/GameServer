...
public:
    ...
    void SetFullLength    (const void * value) { m_FullLength = *(uint32_t *)value; }
    void SetTypeNameLength(uint16_t     value) { m_TypeNameLength = value; }
    void SetTypeNameLength(const void * value) { m_TypeNameLength = *(uint16_t*)value; }
...
private:
    char     m_CheckCode[2];   // 用于验证该包是否是我们规定的游戏协议包
    uint32_t m_FullLength;     // 指示整个包的长度
    uint16_t m_TypeNameLength; //
