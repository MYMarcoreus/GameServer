...
string solveSudoku(const std::string_view & puzzle)
{
...
{
public:
    SudokuServer(EventLoop* loop, IPAddressPtr listenAddr, bool reusPort)
            : server_(loop, listenAddr, reusPort)
    {
        server_.SetConnectionEstablishedCallback(
                std::bind(&SudokuServer::onConnectionEstablished, this, _1));
...
private:
    ...
    void onMessage(TcpConnectionPtr conn, Buffer& buf)
    {
        ...
        while (len >= kCells + 2)
        {
            const char* crlf = std::search(buf.DataBegin(), buf.FreeBegin(), kCRLF, kCRLF+2);
            crlf  = crlf == buf.DataBegin() ? NULL : crlf;

            if (crlf)
            {
                string request(buf.DataBegin(), crlf - buf.DataBegin());
                buf.MoveHead(crlf + 2 - buf.DataBegin());
                len = buf.DataSize();
...
int main(int argc, char* argv[])
{
    ...
    EventLoop loop{true};
    IPAddressPtr listenAddr = std::make_shared<IPv4Address>(9981);
    SudokuServer server(&loop, listenAddr, true);
